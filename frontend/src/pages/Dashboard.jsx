...
import { LayoutGrid, List, Radar, Search, Settings } from 'lucide-react';
...
export default function Dashboard({ setRoute, theme, setTheme }) {
  ...
  return (
    <main className="app-shell min-h-screen bg-radar-bg text-zinc-100">
      <nav className="sticky top-0 z-30 flex h-[65px] items-center gap-4 border-b border-zinc-800 bg-radar-bg/90 px-4 backdrop-blur-xl lg:px-6">
        <button onClick={() => setRoute('/')} className="flex items-center gap-2 text-lg font-extrabold text-white"><Radar className="text-radar-green" size={24} />MarketRadar</button>
        <div className="mx-auto flex h-10 w-full max-w-xl items-center gap-2 rounded-lg bg-radar-panel px-3 ring-1 ring-zinc-800"><Search size={16} className="text-zinc-500" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search all listings" className="w-full bg-transparent text-sm text-white outline-none placeholder:text-zinc-600" /></div>
        ...
        <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="hidden rounded-md px-3 py-2 text-sm font-semibold text-zinc-300 ring-1 ring-zinc-800 hover:bg-zinc-900 sm:block">{theme}</button>
      ...
      <div className="flex">
        ...
        <section className="min-w-0 flex-1 p-4 lg:p-6">
          <div className="mb-4 flex items-center justify-between">
            <div><h1 className="text-xl font-bold text-white">Unified Feed</h1><p className="text-sm text-zinc-500">{filtered.length} listings ranked by signal quality</p></div>
          ...
          <div className={view === 'grid' ? 'grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3' : 'grid grid-cols-1 gap-3'}>
            {filtered.map((listing, index) => <ListingCard key={listing.id} listing={listing} index={index} onSelect={setSelected} />)}
          </div>
        </section>
